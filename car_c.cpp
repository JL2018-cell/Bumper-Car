...
void car::primary_position(){
    srand(time(NULL));
    x = rand()%79+1;
    y = rand()%39+1; // tp avoid the primary positions for p and t to be on the boundaries
}
...
// input the player, no output 
// to get the primary distance between p and t
void car::distance(car & p){
    double dx = p.x - x;
    double dy = q.x - y;
    d1 = sqrt( dx * dx + dy * dy);
}
...
double car::p_speed_beforecol(){
    if(difficulty=="Easy") v1=v0_c;
    if(difficulty=="Medium"||difficulty=="Medium"){
        if(0.5*v0_c*v0_c>u*distance) v1=sqrt(v0_c*v0_c-2*u*distance);
        else v1=0;
    }
    return v1;
}
...
double car::dist_boundary(car & t){
    if(x==t.x){
        if(t.y>y) d2=40-t.y;
        if(t.y<y) d2=t.y;
    }
